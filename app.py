...
def init_db():
    ...
    # Safe migration: add unread tracking to messages without removing existing data.
    message_cols = {r[1] for r in cur.execute("PRAGMA table_info(messages)").fetchall()}
    if "read" not in message_cols:
...
def signup():
    ...
    if len(name) > 80 or len(email) > 160:
...
def create_request():
    ...
    if not receiver:
        conn.close(); return jsonify({"success":False,"message":"Member not found."}),404
    conn.execute("""
        INSERT INTO requests
        (
            sender_id, receiver_id, skill_wanted, skill_offered, message
        )
        VALUES (?,?,?,?,?)
    """, (user["id"], receiver_id, wanted, offered, message))
    conn.execute("INSERT INTO notifications(user_id,title,message,kind) VALUES(?,?,?,?)",(receiver_id,"New skill-swap request",f"{user['name']} sent you a skill-swap request.","request"))
...
def update_request(request_id):
    ...
    conn.execute("INSERT INTO notifications(user_id,title,message,kind) VALUES(?,?,?,?)",(row["sender_id"],"Request updated",f"{user['name']} {action}ed your skill-swap request.","request"))
    conn.commit()
    conn.close()
...
def get_messages(person_id):
    ...
    conn.execute("UPDATE messages SET read=1 WHERE receiver_id=? AND sender_id=?", (user["id"], person_id))
    conn.commit()
    rows = conn.execute("""
        SELECT
            m.*,
...
def send_message(person_id):
    ...
    conn.execute("""
        INSERT INTO messages(sender_id,receiver_id,message,read) VALUES(?,?,?,0)
    """, (user["id"], person_id, message))
    conn.execute("INSERT INTO notifications(user_id,title,message,kind) VALUES(?,?,?,?)",(person_id,"New message",f"{user['name']} sent you a message.","message"))
...
def create_review(person_id):
    ...
    conn.execute("INSERT INTO notifications(user_id,title,message,kind) VALUES(?,?,?,?)",(person_id,"New review",f"{user['name']} left you a {rating}-star review.","review"))
    conn.commit()
    conn.close()
...
def public_profile(person_id):
    conn = get_db()
    row = conn.execute("SELECT * FROM users WHERE id=?", (person_id,)).fetchone()
    conn.close()
    if not row:
...
def dashboard_api():
    ...
    counts={
      ...
      "unread_messages":conn.execute("SELECT COUNT(*) FROM messages WHERE receiver_id=? AND COALESCE(read,0)=0",(uid,)).fetchone()[0],
...
def sessions_api():
    ...
    if request.method=="GET":
        rows=conn.execute("""
            SELECT s.*,
                   ...
                   CASE WHEN s.creator_id=? THEN partner.name ELSE creator.name END AS partner_name
            FROM sessions s
    ...
    d=request.get_json(silent=True) or {}
    raw_partner=d.get("partner_id") or d.get("partnerId") or d.get("guestId") or 0
    try: partner=int(raw_partner)
    except (TypeError,ValueError): partner=0
    title=str(d.get("title","")).strip()
    start=str(d.get("start_at") or d.get("start") or d.get("startsAt") or "").strip()
    raw_duration=d.get("duration", d.get("durationMinutes",60))
    try: duration=int(raw_duration or 60)
    except (TypeError,ValueError): duration=60
    notes=str(d.get("notes","")).strip()

    if not partner or not title or not start:
        conn.close(); return jsonify({"success":False,"message":"Partner, title and date/time are required."}),400
    if partner==uid:
        conn.close(); return jsonify({"success":False,"message":"You cannot schedule a session with yourself."}),400
    if len(title)>120 or len(notes)>1000:
        conn.close(); return jsonify({"success":False,"message":"Session title or notes are too long."}),400
    duration=max(15,min(duration,240))
    partner_row=conn.execute("SELECT id,name FROM users WHERE id=?",(partner,)).fetchone()
    if not partner_row:
        conn.close(); return jsonify({"success":False,"message":"Connection not found."}),404
    connected=conn.execute("""
        SELECT id FROM requests WHERE status='accepted' AND
        ((sender_id=? AND receiver_id=?) OR (sender_id=? AND receiver_id=?)) LIMIT 1
    """,(uid,partner,partner,uid)).fetchone()
    if not connected:
        conn.close(); return jsonify({"success":False,"message":"You can only schedule sessions with an accepted connection."}),403

    conn.execute("INSERT INTO sessions(creator_id,partner_id,title,start_at,duration,notes) VALUES(?,?,?,?,?,?)",(uid,partner,title,start,duration,notes))
    conn.execute("INSERT INTO notifications(user_id,title,message,kind) VALUES(?,?,?,?)",(partner,"New learning session",f"{user['name']} scheduled {title}.","session"))
    conn.commit(); conn.close()
    return jsonify({"success":True,"message":"Session scheduled successfully."}),201
...
def update_session(sid):
    ...
    if not user: return jsonify({"success":False,"message":"Please sign in first."}),401
    d=request.get_json(silent=True) or {}
    status=str(d.get("status") or d.get("action") or "scheduled").strip().lower()
    if status=="reject": status="cancel"
    if status not in {"scheduled","complete","cancel"}:
        return jsonify({"success":False,"message":"Invalid session status."}),400
    conn=get_db()
    row=conn.execute("SELECT * FROM sessions WHERE id=? AND (creator_id=? OR partner_id=?)",(sid,user["id"],user["id"])).fetchone()
    if not row:
    ...
    other=row["partner_id"] if row["creator_id"]==user["id"] else row["creator_id"]
    conn.execute("INSERT INTO notifications(user_id,title,message,kind) VALUES(?,?,?,?)",(other,"Session updated",f"{user['name']} marked '{row['title']}' as {status}.","session"))
    conn.commit(); conn.close(); return jsonify({"success":True,"status":status})
